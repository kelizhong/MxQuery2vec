...
def parse_args():
    ...
    q2v_trainer_parser.add_argument('-pd', '--model-path', default=os.path.join(os.getcwd(), 'data', 'query2vec', 'model'),
                              type=DirectoryType,
    ...
    q2v_trainer_parser.add_argument('-eti', '--enc-test-input', type=str,
                              help='the file name of the encoder input for testing')
    q2v_trainer_parser.add_argument('-dti', '--dec-test-input', type=str,
                              help='the file name of the decoder input for testing')

    q2v_trainer_parser.add_argument('-wll', '--work-load-ist', dest='work_load_list', help='work load for different devices',
    ...
    q2v_trainer_parser.add_argument('decoder_train_data_path', type=str,
                              help='the file name of the decoder input for training')
    q2v_trainer_parser.add_argument('vocabulary_path', default=os.path.join(os.getcwd(), 'data', 'vocabulary', 'vocab.pkl'),
                              type=str,
...
if __name__ == "__main__":
    ...
    if args.action == 'train_query2vec':
        ...
        mxnet_para = mxnet_parameter(kv_store=args.kv_store, hosts_num=args.hosts_num, workers_num=args.workers_num,
                                     ...
                                     load_epoch=args.load_epoch, train_max_samples=args.train_max_samples, word2vec_path=args.word2vec_path)
