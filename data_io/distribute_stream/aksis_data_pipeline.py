...
class AksisDataPipeline(object):
    """Start the raw_data_broker, data_ventilitor_process, parser_worker_process and collector_process.
    the data pipeline is data_ventilitor_process->raw_data_broker->parser_worker_process->collector_process
    ...
    -----
        ...
        not planned to stop. If feed the data to the trainer, the train will continue to train.
        Send the CTRL+C signal will stop all the processes
    """

    def __init__(self, data_dir, vocabulary_path, top_words, file_patterns, batch_size, buckets, worker_num=10,
                 ip=None,
    ...
    def start_collector_process(self, join=False):
        ...
        c = AksisDataCollector(self.ip, self.buckets, self.batch_size, frontend_port=self.collector_fronted_port,
    ...
    def start_parser_worker_process(self):
        ...
        for i in xrange(self.worker_num):
            w = AksisParserWorker(self.ip, self.vocabulary_path, self.top_words,
                                  frontend_port=self.raw_data_backend_port, backend_port=self.collector_fronted_port,
    ...
    def start_data_ventilitor_process(self):
        ...
        for i, (file_pattern, dropout) in enumerate(self.file_patterns):
            v = AksisDataVentilatorProcess(file_pattern, self.data_dir, dropout=dropout, ip=self.ip,
                                           port=self.raw_data_frontend_port, name="aksis_ventilitor_{}".format(i))
            v.start()
