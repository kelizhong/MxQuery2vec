...
def init_log(rank, log_level, log_path):
    logging.basicConfig(format='Node[' + str(rank) + '] %(asctime)s %(levelname)s:%(name)s:%(message)s',
                        level=log_level,
                        datefmt='%H:%M:%S')
    file_handler = logging.FileHandler(os.path.join(log_path, time.strftime("%Y%m%d-%H%M%S") + '.logs'))
    file_handler.setFormatter(logging.Formatter('%(asctime)s [%(levelname)-5.5s:%(name)s] %(message)s'))
    logging.root.addHandler(file_handler)
    head = 'Node[' + str(rank) + '] %(asctime)s %(levelname)s:%(name)s:%(message)s'
